...
function StarIcon(props) {
  return (
    <svg id="star-icon" viewBox="0 0 20 20" aria-hidden="true" {...props}>
    ...
    <div id="star-rating" className="flex">
      {[...Array(5).keys()].map((index) => (
        <StarIcon
          ...
          className={clsx(
            'h-5 w-5',
            rating > index ? 'fill-[#6366F1]' : 'fill-gray-300'
          )}
        />
      ))}
    </div>
...
function Review({ title, body, author, rating, className, ...props }) {
  ...
  return (
    <figure
      ...
      {...props}
    >
      <blockquote id="review-card" className="text-gray-900">
      ...
      <figcaption
        id="reviewer-id"
        className="mt-3 text-sm text-gray-600 before:content-['–_']"
      >
        {author}
      </figcaption>
    </figure>
...
function ReviewGrid() {
  ...
  return (
    ...
    >
      ...
      <div
        id="review-top-fade"
        className="pointer-events-none absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-indigo-200"
      />
      <div
        id="review-bottom-fade"
        className="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-indigo-200"
      />
    </div>
  )
}
...
export function Reviews() {
  return (
    <section
      ...
      className="bg-indigo-200 pt-20 pb-16 sm:pt-32 sm:pb-24"
    >
      <Container>
        <h2
          id="reviews-title"
